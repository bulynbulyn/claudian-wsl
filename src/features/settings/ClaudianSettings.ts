...
export class ClaudianSettingTab extends PluginSettingTab {
  ...
  /**
   * Renders WSL settings section (Windows only).
   * Allows configuring WSL2 mode for running Claude CLI inside WSL.
   */
  private renderWslSettings(containerEl: HTMLElement): void {
    // Only show WSL settings on Windows
    if (process.platform !== 'win32') {
      return;
    }

    new Setting(containerEl).setName('WSL2 Settings').setHeading();

    // WSL mode toggle
    new Setting(containerEl)
      .setName('Enable WSL2 Mode')
      .setDesc('Run Claude CLI inside a WSL2 distribution instead of Windows.')
      .addToggle((toggle) => {
        toggle
          .setValue(this.plugin.settings.wslEnabled ?? false)
          .onChange(async (value) => {
            this.plugin.settings.wslEnabled = value;
            await this.plugin.saveSettings();
            this.display(); // Re-render to show/hide WSL-specific settings
          });
      });

    // Only show WSL-specific settings when WSL mode is enabled
    if (!this.plugin.settings.wslEnabled) {
      return;
    }
    ...
    const distroSetting = new Setting(containerEl)
      ...
      .setDesc('Select the WSL2 distribution to use. Will auto-detect when you click the dropdown.');

    // Create a desc element for status
    const distroStatusEl = containerEl.createDiv({
      cls: 'claudian-wsl-distro-status',
    });
    distroStatusEl.style.fontSize = '0.85em';
    distroStatusEl.style.color = 'var(--text-muted)';
    distroStatusEl.style.marginTop = '-0.5em';
    distroStatusEl.style.marginBottom = '0.5em';

    // Dropdown will be populated asynchronously
    let distroDropdownComponent!: DropdownComponent;

    distroSetting.addDropdown((dropdown) => {
      distroDropdownComponent = dropdown;

      // Add placeholder options initially
      dropdown.addOption('', 'Detecting distributions...');
      dropdown.setValue(this.plugin.settings.wslDistro || '');
    ...
    // Add a button to manually enter distro name if auto-detect fails
    new Setting(containerEl)
      .setName('Manual Distribution Name')
      .setDesc('If auto-detect fails, enter your WSL distribution name here (e.g., Ubuntu).')
      .addText((text) => {
        text
    ...
    // Load WSL distributions asynchronously
    (async () => {
      try {
        const { getWsl2Distros } = await import('../../utils/wslPath');
        const distros = await getWsl2Distros();

        // Clear existing options
        distroDropdownComponent.selectEl.innerHTML = '';

        if (distros.length === 0) {
          distroDropdownComponent.addOption('', 'No WSL2 distributions found');
          distroStatusEl.setText('Check the Obsidian Developer Console (Ctrl+Shift+I) for details.');
          distroStatusEl.style.color = 'var(--text-error)';
          return;
        }

        // Add empty option
        distroDropdownComponent.addOption('', 'Select a distribution...');

        // Add detected distributions
        for (const distro of distros) {
          const label = distro.isDefault ? `${distro.name} (default)` : distro.name;
          distroDropdownComponent.addOption(distro.name, label);
        }

        // Set current value
        distroDropdownComponent.setValue(this.plugin.settings.wslDistro || '');

        // Update status
        const defaultDistro = distros.find(d => d.isDefault);
        if (defaultDistro) {
          distroStatusEl.setText(`Found ${distros.length} WSL2 distribution(s). Default: ${defaultDistro.name}`);
        } else {
          distroStatusEl.setText(`Found ${distros.length} WSL2 distribution(s).`);
        }
      } catch (error) {
        // Show error in dropdown
        distroDropdownComponent.selectEl.innerHTML = '';
        distroDropdownComponent.addOption('', 'Error detecting WSL');

        const errorMsg = error instanceof Error ? error.message : String(error);
        distroStatusEl.setText(`Error: ${errorMsg}`);
        distroStatusEl.style.color = 'var(--text-error)';

        // Also log to console for debugging
        console.error('[Claudian WSL Settings] Error:', error);
      }
    })();

    // WSL CLI path input
    const wslCliPathSetting = new Setting(containerEl)
      .setName('Claude CLI Path (in WSL)')
      .setDesc('Path to Claude CLI inside WSL. Example: /home/user/.local/bin/claude');

    wslCliPathSetting.addText((text) => {
      text
        .setPlaceholder('/home/user/.local/bin/claude')
        .setValue(this.plugin.settings.wslClaudePath || '')
        .onChange(async (value) => {
          this.plugin.settings.wslClaudePath = value.trim();
          await this.plugin.saveSettings();
        });
      text.inputEl.style.width = '100%';
    });
    ...
    new Setting(containerEl)
      ...
      .setDesc('Verify that Claude CLI is accessible in the selected WSL distribution.')
      .addButton((button) => {
        button
          .setButtonText('Test Connection')
          .setCta()
          .onClick(async () => {
            const distro = this.plugin.settings.wslDistro;
            const cliPath = this.plugin.settings.wslClaudePath;

            if (!distro || !cliPath) {
              new Notice('Please select a distribution and enter CLI path first.');
              return;
            }
            ...
            try {
              const { testWslClaudeCli, isWsl2Distro } = await import('../../utils/wslPath');

              // Check if it's WSL2
              const isWsl2 = await isWsl2Distro(distro);
              if (!isWsl2) {
                new Notice(`${distro} is not a WSL2 distribution. Please upgrade it or select another.`);
                return;
              }

              // Test CLI
              const result = await testWslClaudeCli(distro, cliPath);
              if (result.success) {
                new Notice(`Connection successful! Version: ${result.version}`);
              } else {
